...
/**
 * Created by Sneeuwpopsneeuw on 06-Dec-17.
 */
public class Day02b {
    ...
    public Day02b(int[][] input) {
        int sum = 0;

        for (int y=0; y < input.length; y++)
            sum += calculateAdded(input[y]);

        System.out.println(sum);
    }

    private int calculateAdded(int[] array) {
        ...
        for (int i=0; i < array.length; i++) {
            for (int j=0; j < array.length; j++) {
                if (i != j) {
                    ...
                    } else { System.out.println("I: " + array[i] + "   J: " + array[j] + "   = " + (array[i] % array[j])); }
                }
            }
        }
